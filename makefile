!include "../global.mak"

ALL : "$(OUTDIR)\MQ2Posse.dll"

CLEAN :
	-@erase "$(INTDIR)\MQ2Posse.obj"
	-@erase "$(INTDIR)\vc60.idb"
	-@erase "$(OUTDIR)\MQ2Posse.dll"
	-@erase "$(OUTDIR)\MQ2Posse.exp"
	-@erase "$(OUTDIR)\MQ2Posse.lib"
	-@erase "$(OUTDIR)\MQ2Posse.pdb"


LINK32=link.exe
LINK32_FLAGS=kernel32.lib user32.lib gdi32.lib winspool.lib comdlg32.lib advapi32.lib shell32.lib ole32.lib oleaut32.lib uuid.lib odbc32.lib odbccp32.lib $(DETLIB) ..\Release\MQ2Main.lib /nologo /dll /incremental:no /pdb:"$(OUTDIR)\MQ2Posse.pdb" /debug /machine:I386 /out:"$(OUTDIR)\MQ2Posse.dll" /implib:"$(OUTDIR)\MQ2Posse.lib" /OPT:NOICF /OPT:NOREF 
LINK32_OBJS= \
	"$(INTDIR)\MQ2Posse.obj" \
	"$(OUTDIR)\MQ2Main.lib"

"$(OUTDIR)\MQ2Posse.dll" : "$(OUTDIR)" $(DEF_FILE) $(LINK32_OBJS)
    $(LINK32) $(LINK32_FLAGS) $(LINK32_OBJS)


!IF "$(NO_EXTERNAL_DEPS)" != "1"
!IF EXISTS("MQ2Posse.dep")
!INCLUDE "MQ2Posse.dep"
!ELSE 
!MESSAGE Warning: cannot find "MQ2Posse.dep"
!ENDIF 
!ENDIF 


SOURCE=.\MQ2Posse.cpp

"$(INTDIR)\MQ2Posse.obj" : $(SOURCE) "$(INTDIR)"

