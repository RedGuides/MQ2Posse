...
#include "ISXEQPosse.h"

// The mandatory pre-setup function.  Our name is "ISXEQPosse", and the class is ISXEQPosse.
...
// our instanced class.
ISXPreSetup("ISXEQPosse",ISXEQPosse);
...
bool ISXEQPosse::Initialize(ISInterface *p_ISInterface)
...
WriteChatf("ISXEQPosse Loaded");
return true;
}

// shutdown sequence
void ISXEQPosse::Shutdown()
{
    // save settings, if you changed them and want to save them now.  You should normally save
    // changes immediately.
    //pISInterface->ExportSet(ISXEQPosseXML,XMLFileName);

    pISInterface->UnloadSet(ISXEQPosseXML);
...
void ISXEQPosse::ConnectServices()
{
...
void ISXEQPosse::RegisterCommands()
{
    // add any commands
...
void ISXEQPosse::RegisterAliases()
{
    // add any aliases
}

void ISXEQPosse::RegisterDataTypes()
{
    // add any datatypes
    // pMyType = new MyType;
...
void ISXEQPosse::UnRegisterCommands()
{
    // remove commands
    //    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQPosse::UnRegisterAliases()
{
    // remove aliases
}
void ISXEQPosse::UnRegisterDataTypes()
{
    // remove data types
    //if (pMyType)
    //{
...
void ISXEQPosse::UnRegisterTopLevelObjects()
{
    // remove Top-Level Objects
    //    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQPosse::UnRegisterServices()
{
    // shutdown our own services
    //    if (hISXEQPosseService)
    //        pISInterface->ShutdownService(this,hISXEQPosseService);
}

void ISXEQPosse::LoadSettings()
{
    bool bChanged=false;

    // open the settings file, and store the ID number in ISXEQPosseXML
    ISXEQPosseXML=pISInterface->OpenSettings(XMLFileName);
    ...
    Any set can be referred to by its ID.  The base set is stored in ISXEQPosseXML.  Note that the settings
    ...
    */
    ...
    if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQPosseXML,"General"))
    {
